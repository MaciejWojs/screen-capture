...
* feat: add support for dirty rectangle tracking in frame updates and optimize pixel conversion
* feat: add timestamp to FrameUpdate interface and update the frame callback to include timestamp notifications
* feat: enable D3D11 multithread protection and improve frame handling logic
* feat: enhance Linux pixel data fetching and DXGI capture initialization logging
* feat: implement push-based frame delivery for Node/Electron integration
* refactor: improve thread safety and cleanup across Windows capture backends
* fix: add alpha mask handling and pixel format detection for X11 capture
* fix: add AVX2/AVX512 support and fallback handling for pixel conversion
* fix: validate pixel data retrieval and stride handling on Linux
